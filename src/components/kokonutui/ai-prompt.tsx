...
export default function AI_Prompt({setRequest}) {
  ...
  const [selectedModel, setSelectedModel] = useState("DeepSeek R3");
...
// };
  ...
  if(!res.ok) {console.log(data.errors); toast.error(`Serveur error: ${data.details.error.message}`);}
  else {console.log("Réponse du modèle :", data);
  setRequest(data);
  }
                ...
                >
                  <ArrowRight
                    className={cn(
                      ...
                      formData.request.trim() ? "opacity-100" : "opacity-30"
                    )}
                  />
                </button>
              </div>
